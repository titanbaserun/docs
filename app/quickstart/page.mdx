...
A [CLI](/cli) and [PostgreSQL SQL import](/planned/import) are on the roadmap but **not available yet**. The current quickstart path is the browser editor. Today the same operations are available as library functions by importing `@titanbase/core` and the exporter packages directly.
