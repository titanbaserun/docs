...
- `titanbase import` — bring an existing schema into `.titan.json` (the [SQL import](/import) library already ships).
- `titanbase diff` — compare two `.titan.json` versions structurally (the [schema diff](/diff) library already ships).
...
The CLI will package this kind of workflow once the public API is stabilized. The [SQL import](/import) and [schema diff](/diff) libraries are already usable from scripts today.
